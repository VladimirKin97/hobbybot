...
async def handle_steps(message: types.Message):
    ...
    if step == "name":
        ...
        await message.answer("✅ Ваш профіль успішно створено!", reply_markup=main_menu)
        ...
        if message.text == "➕ Створити подію":
            ...
            profile = user_states.get(user_id, {})
            await message.answer(
        ...
        user_states[user_id]["step"] = "completed_event"
        await message.answer("✅ Подію створено! (Поки що без публікації)", reply_markup=main_menu)
...
async def get_photo(message: types.Message):
    ...
    await message.answer("🎯 Вкажіть ваші інтереси через кому (наприклад: футбол, настолки, прогулянки):", reply_markup=back_button)
