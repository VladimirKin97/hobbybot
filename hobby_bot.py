...
async def handle_steps(message: types.Message):
    ...
    if step == "name":
        ...
        if message.text == "➕ Створити подію":
            ...
            phone = users[user_id].get("phone", None)
            users[user_id] = {}
            if phone:
        ...
        await message.answer("🔧 Пошук ще в розробці. Повертаємось у меню.", reply_markup=main_menu)
        user_states[user_id]["step"] = "menu"
...
async def get_photo(message: types.Message):
    ...
    if step == "photo":
        ...
        await message.answer("🎯 Дякую! Тепер вкажіть ваші інтереси через кому (наприклад: футбол, настолки, прогулянки):", reply_markup=back_button)
