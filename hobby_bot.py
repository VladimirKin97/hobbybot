...
async def create_event_steps(message: types.Message):
    ...
    if step == "create_event_title":
        ...
        await message.answer(
            ...
            "📌 *Рекомендація:* Опишіть суть події так, щоб одразу зацікавити потенційних учасників.",
            reply_markup=back_button
        )
        ...
        await message.answer(
            ...
            "⏰ *Рекомендація:* Вкажіть точну дату та час проведення, щоб уникнути непорозумінь.",
            reply_markup=back_button
        )
        ...
        await message.answer(
            ...
            "🌐 *Рекомендація:* Чим точніше вкажете місце, тим простіше буде знайти подію.",
            reply_markup=back_button
        )
        ...
        await message.answer(
            "👥 Вкажіть загальну кількість учасників (наприклад: 10):\n\n"
            "🔢 *Рекомендація:* Вкажіть максимальну кількість людей, щоб уникнути перевантаження.",
            reply_markup=back_button
        )
        ...
        await message.answer(
            "➕ Вкажіть кількість людей, яких ще шукаєте (наприклад: 5):\n\n"
            "🔍 *Рекомендація:* Вкажіть реальну потребу в учасниках.",
            reply_markup=back_button
        )
        ...
        await message.answer(
            "✅ Подію створено! Перевірте деталі перед публікацією:\n\n"
            ...
            f"👥 Учасники: {user_states[user_id]['event_capacity']} (потрібно: {user_states[user_id]['event_needed']})\n\n"
            "✅ Підтвердити / ✏️ Змінити / ❌ Скасувати",
            reply_markup=types.ReplyKeyboardMarkup(
                keyboard=[
                    [types.KeyboardButton(text="✅ Підтвердити")],
                    [types.KeyboardButton(text="✏️ Змінити"), types.KeyboardButton(text="❌ Скасувати")]
                ],
                resize_keyboard=True
            )
        )
