    ...
    async with db_pool.acquire() as conn:
        await conn.execute("""
            ...
            ON CONFLICT (telegram_id) DO UPDATE SET
              phone=EXCLUDED.phone, name=EXCLUDED.name, city=EXCLUDED.city, photo=EXCLUDED.photo, interests=EXCLUDED.interests
...
async def get_request_info(req_id: int):
    async with db_pool.acquire() as conn:
        return await conn.fetchrow("""
            SELECT r.*, e.user_id as organizer_id, e.title as event_title, e.needed_count,
                   u.name as seeker_name
            FROM requests r JOIN events e ON r.event_id = e.id JOIN users u ON r.seeker_id::text = u.telegram_id::text
            WHERE r.id = $1
        """, req_id)

async def update_request_status_db(req_id: int, status: str):
    async with db_pool.acquire() as conn:
        await conn.execute("UPDATE requests SET status = $1 WHERE id = $2", status, req_id)

async def decrement_needed_count(event_id: int):
    async with db_pool.acquire() as conn:
        await conn.execute("UPDATE events SET needed_count = GREATEST(needed_count - 1, 0) WHERE id = $1", event_id)
...
async def get_user_participations(user_id: int):
    async with db_pool.acquire() as conn:
        return await conn.fetch("""
            ...
            WHERE r.seeker_id::text = $1 ORDER BY e.date DESC
        """, str(user_id))
